...
const (
	...
	entryURL            = "https://pc.xiaoheiban.cn"
	loginURL            = `xiaoheiban.cn/login/`
	loginBtnSelector    = `#app > div > div > div > div.footer-main > div > div:nth-child(1) > p`
	loginQRCodeSelector = `#qrcode > img`
...
const (
	...
	// loginFinishSel = `body > ydoc-app > div > div:nth-child(1) > header > div > div > div.top-right > div.own-info > img`
	loginFinishSel = `#nav > div._1nJ7ryw1BkPUB-yg_R7h3h > div._3xgiAvLJejCh-_LxDwiSIw > h1 > span > img`
)
...
func localDownloadPath(subDir ...string) string {
	tmp := make([]string, 0, 10)
	tmp = append(tmp, "download")
	tmp = append(tmp, subDir...)
	return localCacheDir(tmp...)
}
